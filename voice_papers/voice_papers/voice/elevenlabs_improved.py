...
class ElevenLabsImprovedSynthesizer:
    ...
    def synthesize_with_state(
        ...
        state = self._load_state(state_file) if resume and state_file.exists() else {
            "job_id": job_id,
            "status": "started",
            "total_chunks": 0,
            "completed_chunks": [],
        ...
        try:
            ...
            voice_settings = {
                "stability": stability if stability is not None else self.DEFAULT_VOICE_SETTINGS["stability"],
                "similarity_boost": similarity_boost if similarity_boost is not None else self.DEFAULT_VOICE_SETTINGS["similarity_boost"],
                "style": style if style is not None else self.DEFAULT_VOICE_SETTINGS["style"],
                "use_speaker_boost": use_speaker_boost if use_speaker_boost is not None else self.DEFAULT_VOICE_SETTINGS["use_speaker_boost"],
            }
            ...
            print(f"Text split into {len(chunks)} chunks for ~{len(chunks) * 5}-{len(chunks) * 6} minutes total")
            ...
            for i, chunk in enumerate(chunks):
                ...
                if success:
                    ...
                    chunk_info["estimated_credits"] = len(chunk)  # Store original estimate for comparison
                    state["completed_chunks"].append(chunk_id)
                    if request_id:
                        state["request_ids"].append(request_id)
                    state["total_cost"] += cost
                    state["total_credits"] += credits_used
                    print(f"✅ {chunk_id} completed - Cost: ${cost:.4f} | Credits: {credits_used:,} actual (estimated: {len(chunk):,})")
                else:
            ...
            if state["completed_chunks"]:
                ...
                success = self._stitch_chunks(chunks_dir, state["completed_chunks"], output_path)
                
                if success:
                    state["status"] = "completed"
                    state["completed_at"] = datetime.now().isoformat()
                    print(f"✅ Audio saved to: {output_path}")
                    print(f"💰 Total cost: ${state['total_cost']:.4f} | Credits used: {state['total_credits']:,} (actual from API)")
                    print(f"⏱️  Estimated duration: {len(state['completed_chunks']) * 5}-{len(state['completed_chunks']) * 6} minutes")
                else:
            ...
            try:
                ...
                with self.client.text_to_speech.with_raw_response.convert(
                    ...
                    if actual_credits:
                        ...
                        if abs(actual_credits - credits) > credits * 0.1:  # More than 10% difference
                            discount_rate = (credits - actual_credits) / credits * 100
                            print(f"  💸 Credit discount applied: {discount_rate:.1f}% ({credits} estimated → {actual_credits} actual)")
                    else:
    ...
    def _stitch_chunks(self, chunks_dir: Path, chunk_ids: List[str], output_path: Path) -> bool:
    ...
    def _create_cost_report(self, state_dir: Path, state: Dict):
        ...
        try:
            with open(report_path, "w") as f:
                ...
                f.write(f"- Total credits used: {state.get('total_credits', 0):,} (actual API charge)\n")
                f.write(f"- Cost per 1K chars: ${self.PRICING_PER_1K_CHARS.get(self.MODEL_MAP.get(state['model'], 'default'), 0.0009):.4f}\n")
                credits_per_char = state.get('total_credits', 0) / state['total_characters'] if state['total_characters'] > 0 else 0
                f.write(f"- Average credits per character: {credits_per_char:.2f}\n")
                if credits_per_char < 0.7:
                    f.write(f"  👉 You're getting a discount! Flash models charge ~{credits_per_char:.2f} credits/char\n")
                f.write("\n")
                ...
                if state.get('chunks_info'):
                    ...
                    for chunk in state['chunks_info']:
                        ...
                        credits_est = chunk.get('estimated_credits', chunk['characters'])
                        f.write(f"- {chunk['id']}: {chunk['characters']} chars, "
                               f"${chunk.get('cost', 0.0):.4f}, "
                               f"{credits_actual:,} credits")
                        if credits_actual != credits_est:
                            f.write(f" (saved {credits_est - credits_actual:,} credits)")
                ...
                if completed_chunks > 0:
                    avg_credits_per_min = int(state.get('total_credits', 0) / (completed_chunks * 5.5))
                    f.write(f"- Average credits per minute of audio: {avg_credits_per_min:,}\n")
                    
                    # Calculate savings if using Flash model
                    estimated_credits = sum(c.get('estimated_credits', c['characters']) for c in state.get('chunks_info', []))
                    ...
                    if actual_credits < estimated_credits:
                        ...
                        f.write(f"- Total credits saved: {savings:,} ({savings_percent:.1f}% discount)\n")
                        f.write(f"- Cost savings: ${(savings / 1000 * self.PRICING_PER_1K_CHARS.get(self.MODEL_MAP.get(state['model'], 'default'), 0.0009)):.4f}\n")
